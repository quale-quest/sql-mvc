...
https://github.com/cloudera/impala/blob/master/fe/src/main/cup/sql-parser.y
...
exports.exec = function (cx, token_script) {
	...
	var step = function (cx) { //state machine
		...
		var start_transaction = function (cx) {
			//console.log('start_transaction :', cx.dbref);
			try {
			cx.dbref.db.startTransaction(
				function (err, transaction) {
				if (err) {
					console.log('err on start_transaction :', cx.dbref);
					return;
				}
				...
				winston.error('Query-proc start transaction threw: ',e);
			}
		...
		if (cx.sf.ip >= cx.sf.ip_end) { //end of the block
			...
			case 'log': {
					if (!inst[1])
						console.log('sp log ---------------v:', v);
					else {
						console.log('sp log ---------------i:', eval(inst[1]));
					}
					step(cx);
				}
				break;
			...
			case 'if': {
					if (eval(inst[1])) {
						...
						//console.log('sp if (', inst[1] , '==false)');
						if (inst[3]) { //else block
							//console.log('sp else if (', inst[3] , '==true)');
							cx.stack.push(cx.sf);
							cx.sf = {};
							cx.sf.ip = 0;
							cx.sf.bp = inst[3];
						...
						//else console.log('sp if else ', inst[3] , '==false');
					}
					step(cx);
				}
				break;
			...
			case 'query': {
					...
					if (inst[2])
						with_params = inst[2];
					//console.log('sp query : ',inst[1], ' parms:', with_params);
...
var test = function () {
	...
	var jobs_test = [
		["connect"],
		["query", "SELECT first 1 a.POST_PROCEDURE,a.QUERY FROM Z$PK_CACHE a where a.MASTER=? and a.INDX=?", [cid, pkf]],
		["=", " cnt ", " 0 "],
		["log", " r "],
		["if", " v.cnt < 0 ",
			[
	...
	var jobs = [
		...
		["query", "SELECT first 1 a.POST_PROCEDURE,a.QUERY FROM Z$PK_CACHE a where a.MASTER=? and a.INDX=?", [cid, pkf]],
		["log", " r "],
		["adapt", "query", "SELECT p.PKO FROM Z${{r.POST_PROCEDURE}}(2, '{{{r.query}}}' , 'BINTYPE', ?, 'THUMBTYPE', 'THUMB', 'NAME') p", [fs.createReadStream('/tmp/lst.txt')]], //,{ encoding: null})]],
		["complete"]
